...
class PageSettings(BaseModel):
    ...
    gutter_space_cm: float = Field(default=0.4, description="Space between columns in cm")
...
class HeadlineConfig(BaseModel):
    ...
    use_punch_words: bool = Field(default=True, description="Add punch words at beginning")
    ...
    single_line: bool = Field(default=True, description="Single-line vs double-line heading")
...
class WordCountRules(BaseModel):
    ...
    info_box_min: Optional[int] = Field(default=None, description="Minimum words in info box")
    info_box_max: Optional[int] = Field(default=None, description="Maximum words in info box")

class EditorialFilters(BaseModel):
    """Editorial style and content filters"""
    localization: bool = Field(default=True, description="Emphasize local/city relevance")
    ...
    gujarati_tone: bool = Field(default=True, description="Maintain Gujarati newsroom tone")
    highlight_numbers: bool = Field(default=True, description="Bold numbers, percentages, currency")
...
class NewspaperConfig(BaseModel):
    ...
    word_count_rules: Optional[WordCountRules] = Field(default=None, description="Auto-calculated if not provided")
    editorial_filters: EditorialFilters = Field(default_factory=EditorialFilters)
    allowed_sources: List[str] = Field(default_factory=list, description="Whitelisted source domains")
...
class NewspaperSection(BaseModel):
    ...
    section_type: str  # "headline", "headline_cap", "subheading", "intro", "body", "info_box"
    content: str
    word_count: int
